...
import { format, addMinutes, isFuture, set, isPast, isToday, isSameDay } from "date-fns"
...
const ActiveBookingCard = ({ booking, barbershop }: ActiveBookingCardProps) => {
  ...
  const { availableTimes, loading: isTimesLoading } = useDailyAvailability({
    ...
    excludeBookingId: booking.id, // Excluir o booking atual
    currentProfessionalId: booking.professionalId, // Verificar apenas este profissional
  })
  ...
  const timeSlots = useMemo(() => {
    ...
    return TIME_LIST.map((time) => {
      ...
      const timeDate = set(selectedRescheduleDay, { hours: hour, minutes })
      const timeIsInThePast = isPast(timeDate) && isToday(selectedRescheduleDay)
      
      const available = availableTimeSet.has(time) && !timeIsInThePast
  ...
  return (
    <Sheet open={isSheetOpen} onOpenChange={handleSheetOpenChange}>
      ...
      <SheetContent className="w-[85%] max-h-[100vh] flex flex-col">
        ...
        <SheetFooter className="mt-6 border-t pt-4 bg-background">
